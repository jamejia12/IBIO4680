function allBench_fast(imgDir, gtDir, inDir, outDir, nthresh, radius, thinpb)
% allBench_fast(imgDir, gtDir, inDir, outDir, nthresh, maxDist, thinpb)
%
...
parfor i = 1:numel(iids),
    ...
    evFile4 = fullfile(outDir, strcat(fNames, '_ev4.txt'));
    if ~isempty(dir(evFile4)), continue; end
    
    inFile = fullfile(inDir, strcat(fNames, '.mat'));
    gtFile = fullfile(gtDir, strcat(fNames, '.mat'));
    evFile1 = fullfile(outDir, strcat(fNames,'_ev1.txt'));
    evFile2 = fullfile(outDir, strcat(fNames, '_ev2.txt'));
    evFile3 = fullfile(outDir, strcat(fNames, '_ev3.txt'));

    evaluation_bdry_image_fast(inFile,gtFile, evFile1, nthresh, radius, thinpb);
